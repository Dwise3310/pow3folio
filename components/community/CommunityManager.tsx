...
export default function CommunityManager({ userId, initialItems }: Props) {
  ...
  function startEdit(item: CommunityItem) {
    ...
    let category: "built" | "collaboration" | "community" = "collaboration";
    const rawTags = item.tags || [];
    for (const t of rawTags) {
      const k = t.toLowerCase();
      if (k === "built" || k === "built by me") category = "built";
      if (k === "community" || k === "community role") category = "community";
    }
        ...
        <div className={`card space-y-3 ${AUTOFILL_HIGHLIGHT}`}>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-xs text-foreground-muted">
                From CV or link import. Review, then save. You can set category after save.
              </p>
            </div>
          ...
          {drafts.map((d, i) => (
            ...
            >
              ...
              {d.role && <p className="text-xs text-foreground-muted">{d.role}</p>}
            </div>
          ))}
        </div>
      )}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Telegram, Discord, Onchain…"
            />
          </div>
        </div>
        <div>
