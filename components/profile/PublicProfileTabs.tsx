...
function isImageFile(doc: Credential) {
  ...
  return (
    ...
    /\.(png|jpe?g|gif|webp|svg)$/i.test(doc.file_url || "")
  );
}
...
function isPdf(doc: Credential) {
  const t = (doc.file_type || "").toLowerCase();
  const n = (doc.file_name || "").toLowerCase();
  return t.includes("pdf") || n.endsWith(".pdf") || (doc.file_url || "").toLowerCase().includes(".pdf");
}
      ...
      <div key={animKey} className="tab-panel-enter pt-4 pb-8">
        {active === "about" && (
          <div className="space-y-5">
            ...
            {skills.length > 0 && (
              <div>
                <h3 className="mb-1.5 text-[10px] font-medium uppercase tracking-wide text-foreground-subtle">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {skills.map((skill) => (
                    <button
                      ...
                      className="inline-flex items-center rounded-full border border-primary bg-transparent px-2.5 py-1 text-[11px] font-semibold text-white hover:bg-primary/10 transition-colors"
                    >
                      {skill.name}
                    </button>
                  ))}
            ...
            {workExperience.length > 0 && (
              <div>
                <h3 className="mb-2 text-[10px] font-medium uppercase tracking-wide text-foreground-subtle">
                  Work experience
                </h3>
                  ...
                  {credentials.map((doc) => {
                    ...
                    return (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="group flex flex-col overflow-hidden rounded-xl border border-border bg-surface/40 transition-all hover:border-primary/40"
                      >
                        <div className="aspect-[4/3] w-full bg-surface-elevated flex items-center justify-center overflow-hidden">
                          {img ? (
                            ...
                            <div className="flex flex-col items-center gap-1 text-foreground-subtle">
                              <span className="text-2xl font-bold tracking-tight text-primary/80">
                                PDF
                              </span>
                        ...
                        <div className="px-2.5 py-2 border-t border-border/60">
                          ...
                          {(doc.issuer || doc.file_name) && (
                            <p className="mt-0.5 text-[10px] text-foreground-subtle line-clamp-1">
          ...
          <div className="space-y-4">
            ...
            {community.length === 0 ? (
              <div className="card text-sm text-foreground-subtle">
                No projects or collaborations yet.
              </div>
            ) : (
          ...
          <div>
            {writings.length === 0 ? (
              ...
              <div className="tight-cards tight-cards-3">
                {writings.map((w) => {
                  const imgs = [w.thumbnail_url, w.image_url_2].filter(
                    (u): u is string => !!u
                  );
                  return (
                    <article
                      key={w.id}
                      className="card flex h-auto w-full flex-col overflow-hidden p-0 transition-all duration-200 hover:border-primary/40 hover:-translate-y-0.5 hover:shadow-md"
                    >
                      {imgs.length > 0 && (
                        <TradeImageCarousel
                          images={imgs}
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <span className="inline-flex rounded-full border border-primary px-2.5 py-1 text-xs font-semibold text-white">
                {skillPopup.name}
              </span>
              <button
                type="button"
            ...
            <p className="mt-3 text-sm leading-relaxed text-foreground-muted whitespace-pre-wrap">
              {skillPopup.description?.trim() || "No description added for this skill yet."}
            </p>
