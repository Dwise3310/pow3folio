    ...
    fetch(`/api/onchain/${activeWallet}${qs}`, { signal: ctrl.signal })
      ...
      .catch((e) => {
        ...
        if (e instanceof DOMException && e.name === "AbortError") {
          setError("Onchain lookup timed out. Try again — healthy chains still load independently.");
        } else {
        ...
        <>
          <div className="relative">
            ...
            <button
              ...
              className="flex w-full items-center justify-between gap-2 rounded-xl border border-border bg-surface-elevated px-3 py-2.5 text-left text-sm font-medium transition-colors hover:border-primary/40"
              aria-expanded={chainOpen}
            >
              <span className="truncate">{selected?.name || "Select chain"}</span>
              <svg
                className={`h-4 w-4 shrink-0 text-foreground-muted transition-transform duration-200 ${chainOpen ? "rotate-180" : ""}`}
                viewBox="0 0 20 20"
                fill="currentColor"
                aria-hidden
              >
            ...
            <div
              className={`overflow-hidden transition-all duration-200 ease-out ${
                chainOpen ? "mt-1.5 max-h-72 opacity-100" : "max-h-0 opacity-0"
              }`}
            >
                ...
                {shownChains.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span className={`block truncate text-sm font-medium ${isActive ? "text-primary" : ""}`}>
                          {c.name}
                        </span>
                        <span className="block text-[10px] text-foreground-subtle tabular-nums">
                          {c.txCount > 0 ? `${c.txCount.toLocaleString()} tx` : c.imported ? "Imported" : "—"}
                          {Number(c.balance) > 0 ? ` · ${c.balance} ${c.nativeSymbol}` : ""}
                        </span>
                      </button>
                      {owner && (
                        <button
                          type="button"
                          role="switch"
                          aria-checked={publiclyOn}
                          onClick={(e) => {
                            e.stopPropagation();
                            togglePublicChain(c.id);
                          }}
                          className={`relative h-5 w-9 shrink-0 rounded-full transition-colors ${
                            publiclyOn ? "bg-primary" : "bg-border"
                          }`}
                          title={publiclyOn ? "Visible on public page" : "Hidden on public page"}
                        >
                          <span
                            className={`absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${
                              publiclyOn ? "translate-x-4" : "translate-x-0"
                            }`}
                          />
                        </button>
                      )}
            ...
            <p className="mt-1.5 text-[11px] text-foreground-subtle">
              {owner
                ? "Pick a chain to view stats. Toggle on the right controls public visibility. At least one chain must stay public."
                : "Interacted and imported networks. Open the menu to switch chains."}
            </p>
