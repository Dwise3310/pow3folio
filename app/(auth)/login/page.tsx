...
export default function LoginPage() {
  ...
  async function connectWallet() {
    ...
    try {
      ...
      // Wallet login requires a signed message (SIWE) backend.
      // For now: store address and prompt email login, or use after session exists.
      const supabase = createClient();
      const {
        data: { user },
      ...
      if (user) {
        ...
        setError(
          `Wallet ${address.slice(0, 6)}…${address.slice(-4)} connected. Sign in with email/Google/X first, then reconnect wallet from Profile to save it. Full one-click wallet login (SIWE) comes in a later phase.`
        );
      }
    } catch (err) {
