...
export default function ProfileForm({ profile, email, linkedProviders }: Props) {
  ...
  useEffect(() => {
    const pending = loadPendingAutofill();
    if (!pending || !pending.pending.includes("profile")) return;
    const data = pending.data as AutofillPayload;
    const marks: Record<string, boolean> = {};
    setForm((prev) => {
      const next = { ...prev };
      if (data.display_name?.trim()) {
        next.display_name = data.display_name.trim();
        marks.display_name = true;
      }
      if (data.bio?.trim()) {
        next.bio = data.bio.trim();
        marks.bio = true;
      }
      if (data.long_bio?.trim()) {
        next.long_bio = data.long_bio.trim();
        marks.long_bio = true;
      }
      if (data.website_url?.trim()) {
        next.website_url = data.website_url.trim();
        marks.website_url = true;
      }
      if (data.github_url?.trim()) {
        next.github_url = data.github_url.trim();
        marks.github_url = true;
      }
      if (data.x_url?.trim()) {
        next.x_url = data.x_url.trim();
        marks.x_url = true;
      }
      if (data.telegram_url?.trim()) {
        next.telegram_url = data.telegram_url.trim();
        marks.telegram_url = true;
      }
      if (data.location_country?.trim()) {
        next.location_country = data.location_country.trim();
        marks.location = true;
      }
      if (data.location_region?.trim()) {
        next.location_region = data.location_region.trim();
        marks.location = true;
      }
      return next;
    });
    ...
    setHl(marks);
    setSaveMsg("Amber fields came from Import on the dashboard. Review, then Save profile.");
  }, []);
  ...
  function addSkill() {
    ...
    const description = skillDesc.trim().slice(0, 250);
    if (!name) return;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-7 overflow-x-hidden">
      {Object.keys(hl).length > 0 && (
        <p className="text-xs text-amber-500 rounded-lg border border-amber-400/40 bg-amber-400/10 px-3 py-2">
          Amber outline = filled from Import on the dashboard. Review, then Save. Unsaved import stays if you leave and come back (until browser refresh).
        </p>
      )}
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input id="display_name" className={`input ${hl.display_name ? AUTOFILL_HIGHLIGHT : ""}`} value={form.display_name} onChange={(e) => update("display_name", e.target.value)} />
          </div>
        </div>
        <div>
          <label className="label" htmlFor="bio">Short bio</label>
          <textarea id="bio" className={`input min-h-[80px] resize-y ${bioHasLink ? "border-danger focus:border-danger focus:ring-danger" : hl.bio ? AUTOFILL_HIGHLIGHT : ""}`} value={form.bio} onChange={(e) => update("bio", e.target.value)} maxLength={160} placeholder="One-line intro, no links" />
        ...
        <div>
          ...
          <textarea id="long_bio" className={`input min-h-[100px] ${hl.long_bio ? AUTOFILL_HIGHLIGHT : ""}`} value={form.long_bio} onChange={(e) => update("long_bio", e.target.value)} placeholder="Tell people what you do in Web3…" />
        </div>
      </section>

      <section className={`space-y-4 ${hl.skills ? "rounded-xl p-3 " + AUTOFILL_HIGHLIGHT : ""}`}>
        <h2 className="text-sm font-semibold tracking-tight border-b border-border pb-2">Skills / Service pillars</h2>
        <p className="text-xs text-foreground-subtle">Name + description (max 250 chars). Max 12 pillars. On the public profile, tap a chip to read the full brief.</p>
        ...
        <div className="space-y-2 rounded-xl border border-border p-3">
          ...
          <textarea className="input text-sm min-h-[80px]" value={skillDesc} onChange={(e) => setSkillDesc(e.target.value.slice(0, 250))} placeholder="Brief explanation (max 250 characters)" maxLength={250} />
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-foreground-subtle">{skillDesc.length}/250</span>
