...
/** Recurse strings, arrays, and plain objects (Notion nests annotations as objects). */
...
async function fetchNotionText(url: URL): Promise<string | null> {
  ...
  // Page id is usually the last 32 hex chars in the path (with or without dashes)
...
async function fetchGitHubText(url: URL): Promise<string | null> {
  ...
  if (!readme) {
    try {
      const rm = await fetch(
        `https://raw.githubusercontent.com/${encodeURIComponent(user)}/${encodeURIComponent(user)}/main/README.md`,
        { signal: AbortSignal.timeout(8000) }
      );
      if (rm.ok) readme = (await rm.text()).slice(0, 15000);
    } catch {
...
- Map founder roles, community building, Discord/Telegram mods, campaign leads, testnet contributions, airdrop work, and project launches into "community" (these become Projects/Collab on the profile). Use title like the community or project name, role like Founder / Community Lead / Contributor.
- Map articles, threads, research posts, docs into "writings" when URLs exist.
- Map trading strategies or technical skills into "skills" with short descriptions.
- Prefer concrete facts only. Omit unknown keys. Be thorough — extract every clear role, community, and project mentioned.`;
