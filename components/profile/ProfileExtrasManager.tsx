...
import { loadPendingAutofill } from "@/lib/autofill-store";
...
export default function ProfileExtrasManager({
  ...
  async function persist(nextWork: WorkExperience[], nextEdu: Education[]) {
    ...
    const { error } = await supabase
      .from("profiles")
      .update({ work_experience: nextWork, education: nextEdu })
      .eq("id", userId);
    setSaving(false);
    if (error) {
      setErr(
  ...
  useEffect(() => {
        ...
        const mapped: WorkExperience[] = incoming
          ...
          .map((w) => ({
            ...
            description: w.description
              ? String(w.description).trim().slice(0, 300)
              : null,
            url: w.url ? absoluteUrl(String(w.url)) : null,
            employment_type:
              w.employment_type === "part-time" ? "part-time" : "full-time",
            start_date:
              toMonth(w.start_date) || new Date().toISOString().slice(0, 7),
        ...
        queueMicrotask(() => {
          setEdu((eduPrev) => {
            ...
            const mappedEdu: Education[] = incomingEdu
              ...
              .map((e) => ({
                ...
                field_of_study: e.field_of_study
                  ...
                  : null,
                country: e.country
                  ? String(e.country).trim().slice(0, 60)
                  : null,
            ...
            void persist(next, eduNext).then((ok) => {
              if (ok) {
                setMsg(
                  "Work and education filled from import. Review the cards below."
                );
              }
            });
            return eduNext;
    ...
    if (
      pending?.pending.includes("profile") &&
      (pending.data.work_experience?.length || pending.data.education?.length)
    ) {
      onAutofill(
        new CustomEvent("pow3-autofill-extras", {
          detail: {
            work_experience: pending.data.work_experience ?? [],
            education: pending.data.education ?? [],
          },
        })
      );
    }
  ...
  async function addWork() {
    ...
    if (!wForm.url.trim()) {
      setErr("Add a link to the company or project before saving this experience.");
      return;
    }
    const link = absoluteUrl(wForm.url);
    if (!link) {
      setErr("Add a valid link to the company or project before saving this experience.");
      return;
    }
    if (work.length >= 5) {
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-foreground-muted">
              Max 5 entries. Company or project link is required for each entry.
            </p>
          </div>
        ...
        {work.length === 0 && !showWorkForm && (
          <p className="text-xs text-foreground-subtle rounded-lg border border-dashed border-border px-3 py-4 text-center">
            No work entries yet. Import a CV or link on the dashboard, or add manually.
          </p>
        )}
              ...
              <div className="min-w-0">
                ...
                {w.url && (
                  <a
                    ...
                    className="text-[11px] text-primary break-all"
                  >
                    {w.url}
                  </a>
                )}
        ...
        {showWorkForm && (
          <div className="rounded-xl border border-border p-3 space-y-3">
            ...
            <div>
              <label className="label text-xs">Company / project link *</label>
              <input
                className={`input text-sm ${!wForm.url.trim() ? "border-danger/60" : ""}`}
                value={wForm.url}
              ...
              <p className="mt-1 text-[11px] text-foreground-subtle">
                Required. Link to the company site, product, or campaign page.
              </p>
            </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-foreground-muted">
              Degrees and studies. Shown under About.
            </p>
          </div>
          {!showEduForm && (
            <button
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEForm({ ...eForm, field_of_study: e.target.value })
                  }
                />
              </div>
            </div>
